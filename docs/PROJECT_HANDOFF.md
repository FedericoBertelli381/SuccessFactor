...
STEP 43 completato: Performance dashboard aggregata. Descrizione: Aggiunta la pagina `/hr/performance-dashboard` read-only per HR/admin, con KPI ciclo aggregati, completion participant, progress medio goal, completion assessment, score medio, distribuzione score e breakdown per OrgUnit e JobRole, preservando tenant e senza cambiare i flow My*, Team* o HR detail.
